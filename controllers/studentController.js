    ...
    con.query(query, function (error, result, fields) {
      if (error) {
        ...
        if (result.length > 0) {
          ...
          res.sendFile(
            path.join(__dirname, "..", "student", "student_dashboard.html")
          );
        } else {
          console.log("Login Not Found");
          res.sendFile(
            path.join(__dirname, "..", "student", "student_signup.html")
          );
        }
      }
    });
...
con.query('INSERT INTO student(name,username,email,phonenumber,password,qualification,age,image) VALUES(?,?,?,?,?,?,?,?)',[name_,username,email,phone,password,qualification,age,file_name],(error,result)=>{
  ...
  forgetverificationcode: (req, res) => {
    res.sendFile(
      path.join(__dirname, "..", "student", "forgetverificationcode.html")
    );
  },
  ...
  updatepasswordfile: (req, res) => {
    res.sendFile(
      path.join(__dirname, "..", "student", "updatestudentpassword.html")
    );
  },
